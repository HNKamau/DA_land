...
land_consolidation_function <- function(x, varnames)
{
  ...
  off_farm_income <- vv(off_farm_income_kes_pm, gen_CV, n_years, relative_trend = inflation_rate) * 12   # pm is per month | CHANGED: shares the inflation trend for consistency with other nominal income (see note 1)
